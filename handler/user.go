...
func (h *userHandler) Login(c *gin.Context) {
	...
	formatter := user.FormatUser(loggedinUser, "oooooo")
...
func (h *userHandler) CheckEmailAvailability(c *gin.Context) {
	...
	metaCode := http.StatusForbidden
	metaStatus := "error"

	if isEmailAvailable {
		metaMsg = "Email is available"
		metaCode = http.StatusOK
		metaStatus = "success"
	}

	response := helper.APIResponse(metaMsg, metaCode, metaStatus, data)
	c.JSON(http.StatusOK, response)
}
...
func (h *userHandler) UploadAvatar(c *gin.Context) {
	...
	// Get the value from jwt, be patient :)
	userID := 1
