...
func (h *userHandler) Create(c *gin.Context) {
	var inputUser user.FormCreateUserInput

	err := c.ShouldBind(&inputUser)
	if err != nil {
		inputUser.Error = err
		c.HTML(http.StatusOK, "user_new.html", inputUser)
		return
	}

	registerInput := user.RegisterUserInput{}
	registerInput.Name = inputUser.Name
	registerInput.Email = inputUser.Email
	registerInput.Occupation = inputUser.Occupation
	registerInput.Password = inputUser.Password
